...
/**
 * Data Stucture data - title, generatedTime - sheets -sheet1 - title (required)
 * - headers - name - width - type - dataRows - List of objects/values
 */
@Service
	...
	public File generateExcelReport(ExcelData data) throws IOException {
		...
		for (ExcelDataSheet sheetData : data.getSheets()) {
			...
			for (int i = 0; i < headersData.size(); i++) {
				...
				if (headerData.getWidth() > 0)
					sheet.setColumnWidth(i, headerData.getWidth());
				headerCell.setCellValue(headerData.getName());
